...
```
I believe that society’s definitions of social structures are deeply embedded into most countries’ way to organize identification databases. For example, it is expected that a person should have a residence with a permanent address. Having an address is therefore required for most procedures related to legal documents and taxes. This can obviously cause issues for homeless people (you need to have identification papers in order to work, but you need to have a home and an address to have papers, and you also need to work to have money for an apartment). It can also cause issues for nomadic people, who may decide to move a lot in their lives, or people separating their lives across multiple countries. In a similar way, it is expected that one has a phone, and therefore, phone numbers are required for most services nowadays and embedded in most client databases.
As another example, the concept of fairness is at the basis of data systems such as universal healthcare, where services are provided based on need, and this service is not considered something that should be sold or commercialized. In addition, it does not depend on other social factors such as wealth, employment status, or citizenship. 
Finally, I believe that trust and responsibility are other values used in many data systems, for example the credit score system, banks, and loan services. Based on how one handle its money and correctly pays his debts, trust is built, which will give people more opportunities and access to more financial products but will also determine other critical life aspect such as renting an apartment. This highlight the idea that a society’s values are at the basis of all the systems in place to regulate and organize people’s lives. 
```
